...
if flood_mode:
    ...
    for i in range(pub_count, pub_count + sub_count):
        ...
        cmd_str = f'python3 {src_dir}/subscriber.py --net_size={host_count} --port={xin} --topic={topic} --label={mLabel} >logs/h{i+1}.log &'
        print(cmd_str)
        net.hosts[i].cmd(cmd_str)
    ...
    # sub.py <proxy_interface> <interface_port (proxy publish port)> <topic>
    for i in range(pub_count + 1, pub_count + sub_count + 1):
        topic = randrange(1e4, 1e5)
        cmd_str = f'python3 {src_dir}/subscriber.py --interface={x_intf} --port={xout} --topic={topic} --label={mLabel} >logs/h{i+1}.log &'
        print(cmd_str)
        net.hosts[i].cmd(cmd_str)
